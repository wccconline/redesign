...
**Social previews:** `index.html` has static Open Graph / Twitter card tags (title, description, and `public/images/social-card.png`, a 1200×630 card made from the logo). They are site-wide, not per-page, because link-preview crawlers (Facebook, iMessage, Slack, X) don't run JavaScript, so every shared link shows the home page's title and description. Per-page previews would need prerendering. `og:url` is deliberately omitted so each shared page keeps its own URL.
